...
class Toontown(commands.Cog):
    def __init__(self, bot):
        ...
        self.boss_bots = ["Flunky", "Pencil Pusher", "Yesman", "Micromanager", "Downsizer", "Head Hunter", "Corporate Raider", "The Big Cheese"]
        self.law_bots = ["Bottom Feeder", "Bloodsucker", "Double Talker", "Ambulance Chaser", "Back Stabber", "Spin Doctor", "Legal Eagle", "Big Wig"]
        self.cash_bots = ["Short Change", "Penny Pincher", "Tightwad", "Bean Counter", "Number Cruncher", "Money Bags", "Loan Shark", "Robber Baron"]
        self.sell_bots = ["Cold Caller", "Telemarketer", "Name Dropper", "Glad Hander", "Mover & Shaker", "Two-Face", "The Mingler", "Mr. Hollywood"]

        self.toontown_jokes = [['What goes TICK-TICK-TICK-WOOF?', 'A watchdog! '],
                                    ...
                                    ['Why did Micky Mouse take a trip to outer space?', 'He wanted to find Pluto.']]

    @commands.command()
    async def invasions(self, ctx):
        r = requests.get("https://toonhq.org/api/v1/invasion/")
        data = r.json()
        cog_invasions = data["invasions"]

        invasion_embeds = []

        for invasion in cog_invasions:
            # Fix weird cog names
            cog_name = invasion["cog"].replace('\u0003', '')

            # Set the cog image url based on the cog's name and manually format the edge cases
            cog_img_url = "https://toonhq.org/static/2.4.4/img/cogs/{}.png".format(('_'.join(cog_name.split())).lower())

            if cog_name == "Back Stabber":
                cog_img_url = "https://toonhq.org/static/2.4.4/img/cogs/backstabber.png"
            elif cog_name == "Yesman":
                cog_img_url = "https://toonhq.org/static/2.4.4/img/cogs/yes_man.png"
            elif cog_name == "Two-Face":
                cog_img_url = "https://toonhq.org/static/2.4.4/img/cogs/two_face.png"
            elif cog_name == "Mover & Shaker":
                cog_img_url = "https://toonhq.org/static/2.4.4/img/cogs/mover_and_shaker.png"
            elif cog_name == "Mr. Hollywood":
                cog_img_url = "https://toonhq.org/static/2.4.4/img/cogs/mr_hollywood.png"
            elif "(Skelecog)" in cog_name:
                cog_img_url = "https://toonhq.org/static/2.4.4/img/cogs/skelecog.png"

            # Set author image url for embed based on cog type
            if cog_name.replace("(Skelecog)", '').strip() in self.boss_bots:
                author_icon_url = "https://vignette.wikia.nocookie.net/toontown/images/1/1f/Bossbot-48x48.gif"
                cog_name += " [Bossbot]"
            elif cog_name.replace("(Skelecog)", '').strip() in self.law_bots:
                author_icon_url = "https://vignette.wikia.nocookie.net/toontown/images/1/18/Lawbot-96x96.gif"
                cog_name += " [Lawbot]"
            elif cog_name.replace("(Skelecog)", '').strip() in self.cash_bots:
                author_icon_url = "https://vignette.wikia.nocookie.net/toontown/images/3/37/Cashbot-96x96.gif"
                cog_name += " [Cashbot]"
            elif cog_name.replace("(Skelecog)", '').strip() in self.sell_bots:
                author_icon_url = "https://vignette.wikia.nocookie.net/toontown/images/8/83/Sellbot.jpg"
                cog_name += " [Sellbot]"
            else:
                author_icon_url = "https://ttr-invasion-tracker.herokuapp.com/static/images/logo.png"

            # Calculate remaining time in seconds
            # Formula: round((total_cogs - defeated_cogs) / defeat_rate)
            remaining_seconds = round((invasion["total"] - invasion["defeated"]) / invasion["defeat_rate"])

            invasion_embed = discord.Embed(
                colour = discord.Colour.purple()
            )

            invasion_embed.set_footer(text = "Started at {}".format(time.strftime("%I:%M %p", time.localtime(invasion["start_time"]))))
            invasion_embed.set_thumbnail(url = cog_img_url)
            invasion_embed.set_author(name = "Invasion", icon_url = author_icon_url)
            
            invasion_embed.add_field(name = "Cog", value = cog_name, inline = True)
            invasion_embed.add_field(name = '\u200b', value = '\u200b', inline = True)
            invasion_embed.add_field(name = "District", value = invasion["district"], inline = True)

            if invasion["total"] != 1000000:
                invasion_embed.add_field(name = "Amount", value = "{} / {} Cogs".format(invasion["defeated"], invasion["total"]), inline = True)
                invasion_embed.add_field(name = '\u200b', value = '\u200b', inline = True)
                invasion_embed.add_field(name = "Time", value = "{} Remaining".format(str(datetime.timedelta(seconds=remaining_seconds))), inline = True)
            else:
                invasion_embed.title = "Mega Invasion!"

            invasion_embeds.append(invasion_embed)
            
        paginator = BotEmbedPaginator(ctx, invasion_embeds)
    ...
    @commands.command()
    async def toontownjoke(self, ctx):
        random_joke = random.choice(self.toontown_jokes)

        toontown_joke_embed = discord.Embed(
            title = random_joke[0],
            description = random_joke[1],
            colour = discord.Colour.purple()
        )

        toontown_joke_embed.set_author(name = "ToonTown Joke", icon_url = "https://vignette.wikia.nocookie.net/toontown/images/a/a5/332.png")
        toontown_joke_embed.set_thumbnail(url = "https://vignette.wikia.nocookie.net/toontownrewritten/images/8/85/Eye-icon_2.png")

        await ctx.send(embed = toontown_joke_embed)
